	...
	private class AddNewProductHandler implements EventHandler<ActionEvent> {
		public void handle(ActionEvent evt) {
			...
			if(addProductPopup.getName().getText().trim().equals("")) 
				...
				try{
					CatalogPres selectedCatalog = ManageProductsData.INSTANCE.getSelectedCatalog();
					Catalog cat = selectedCatalog.getCatalog();
					...
					boolean result = ManageProductsData.INSTANCE.addToProdList(
							selectedCatalog, prodPres);
					
					if(result){
						maintainProductsWindow.getTable().setItems(
								ManageProductsData.INSTANCE.getProductsList(selectedCatalog));
					}
