...
public enum CheckoutUIControl {
	...
	private class ProceedFromCartToShipBill implements EventHandler<ActionEvent>, Callback {
		...
		public void doUpdate() {
			// System.out.println("testing..."+ShoppingCartWindow.INSTANCE.getTable().getItems().get(0).getCartItem().getQuantity());
		...
		public void handle(ActionEvent evt) {
			...
			try {
				...
				BrowseSelectUIControl.INSTANCE.runQuantityRules(
						BrowseSelectData.INSTANCE.getSelectedProduct().getProduct(),
						BrowseSelectData.INSTANCE.getSelectedProduct().getProduct().getQuantityAvail() + "");
			} catch (RuleException e) {
				// handle
				rulesOk = false;
				ShoppingCartWindow.INSTANCE.displayError("Shopping cart error : " + e.getMessage());
	...
	private class SubmitHandler implements EventHandler<ActionEvent> {
		...
		public void handle(ActionEvent evt) {
			...
			List<CartItemPres> cartData = BrowseSelectData.INSTANCE.getCartData();

			try {
				CheckoutController.INSTANCE.runFinalOrderRules(shoppingCart);
				
				for (CartItemPres pres : cartData)
					liveCartItems.add(new CartItemImpl(pres.getCartItem().getItemName(),
							pres.getCartItem().getQuantity() + "", 
							pres.getCartItem().getTotalPrice() + ""));
				
				shoppingCart.getLiveCart().setCartItems(liveCartItems);
