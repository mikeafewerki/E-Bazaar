...
public enum ManageProductsData {
	...
	private CatalogPres readDefaultCatalogFromDataSource() {
		...
		return catalogList.get(0); //this assuming that catalogList has more than 1 element
	}
	...
	private ObservableMap<CatalogPres, List<ProductPres>> readProductsFromDataSource(){
		...
		//This depends upon that catalogList is initialized first
		ObservableMap<CatalogPres, List<ProductPres>> pMap = DefaultData.PRODUCT_LIST_DATA;
		try{
			for(CatalogPres catPres: catalogList){
				List<Product> prods = ManageProductsController.INSTANCE.getProductsList(catPres.getCatalog().getName());
				List<ProductPres> productPreses = new ArrayList<>();
				for(Product p: prods){
					ProductPres pp = new ProductPres();
					pp.setProduct(p);
					productPreses.add(pp);
				}
				pMap.put( catPres, FXCollections.observableList(productPreses));
			}
		}
	...
	public boolean addToProdList(CatalogPres catPres, ProductPres prodPres) {
		...
		return specifiedProds.addAll(newProducts);
	...
	private ObservableList<CatalogPres> readCatalogsFromDataSource() {
		...
		try{
			List<Catalog> catalogs = ManageProductsController.INSTANCE.getCatalogList();
